...
namespace PhysicsEngine
{
	...
	void DynamicActor::SetAngularDamping(PxReal value)
	{
		((PxRigidDynamic*)actor)->setAngularDamping(value);
	}

	void DynamicActor::SetLinearDamping(PxReal value)
	{
		((PxRigidDynamic*)actor)->setLinearDamping(value);
	}
	...
	void Scene::Init()
	{
		...
		sceneDesc.filterShader = this->filter_shader;
		...
		//default gravity
		px_scene->setGravity(PxVec3(0.0f, -15.0f, 0.0f));
	...
	PxParticleSystem* Scene::CreateParticles(PxU32 maxParticles, bool perParticleRestOffset)
	{
	...
	void Scene::HighlightOn(PxRigidDynamic* actor)
	{
		...
		PxU32 test = actor->getShapes((PxShape**)&shapes.front(), (PxU32)shapes.size());
